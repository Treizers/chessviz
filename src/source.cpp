...
void moveFigure(char turn[], char pole[][9])
{
    if (pole[NumberBoard1 - 1][LetterBoard1] == turn[0])
    {
      ...
      cout << "Entry incorrect, you tried to go not by figures from the back row" << endl << endl;
}
...
bool Check(char turn[], char pole[][9], int Qualifier)
{
    ...
    if (NumberBoard1 <= BoundMin || NumberBoard1 >= BoundMax || LetterBoard1 <= BoundMin || LetterBoard1 >= BoundMax || NumberBoard2 <= BoundMin || NumberBoard2 >= BoundMax || LetterBoard2 <= BoundMin || LetterBoard2 >= BoundMax)
    {
